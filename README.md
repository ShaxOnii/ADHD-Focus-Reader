# ADHD-Focus-Reader 🧠✨
🇬🇧 EN: A smart Chrome extension designed for people with ADHD. It uses Bionic Reading principles to bold words and plays context-aware ambient music to improve focus.  [Read in English](#english) 

🇵🇱 PL: Inteligentna wtyczka do Chrome dla osób z ADHD. Ułatwia czytanie dzięki technice pogrubiania liter (Bionic Reading) i odtwarza muzykę tła dopasowaną do czytanego tekstu.  [Czytaj po polsku](#polski)

<a id="english"></a>
## 🇬🇧 English
An intelligent Google Chrome extension designed for people with ADHD and concentration difficulties. The tool combines visual reading aids with an audio module, creating an optimal environment for reading and processing long texts online.
### 🌟 Key Features
- **Intelligent Text Bolding (Bionic Reading)**  
  The extension automatically analyzes text blocks on web pages and bolds specific letters in words (e.g., the first few letters). This helps "guide" your eyes through the text and allows the brain to quickly auto-complete words, reducing reading fatigue. You can choose in the menu whether to bold letters at the beginning, at the end, or on both sides of the word.
  
- **Visual Noise Filtering**  
  The script deliberately ignores large headings (H1-H6), buttons, and navigation links. It only modifies the text that actually requires focus (like paragraphs and long articles), preventing visual overstimulation.
- **Context-Aware Background Music (Offline AI)**  
  The extension features a built-in keyword analyzer. It instantly scans the text for thematic context and plays a suitable, looping background sound based on the results (e.g., "fantasy ambience" for fiction or "ocean waves / lo-fi" for scientific articles). This works completely offline, without sending any of your data to external servers.
### 🛠️ Installation for Testers
To test the extension locally in your browser:
1. Download this repository to your computer.
2. Open Google Chrome and go to: `chrome://extensions/`
3. Enable **Developer mode** in the top right corner.
4. Click **Load unpacked** in the top left corner.
5. Select the folder containing the extension. Done! (We recommend pinning the puzzle icon to your toolbar).
### ⚙️ Technologies
- Vanilla JavaScript, HTML5, CSS3
- Google Chrome Extension API (Manifest V3)
- Chrome Offscreen API (for continuous background audio playback)
---
*A project created for better and calmer knowledge acquisition in the digital world.*
---

<a id="polski"></a>
## 🇵🇱 Polski
Inteligentna wtyczka do przeglądarki Google Chrome, zaprojektowana z myślą o osobach z ADHD i zaburzeniami koncentracji. Narzędzie łączy ułatwienia wizualne z modułem dźwiękowym, tworząc optymalne środowisko do czytania i przyswajania długich tekstów w Internecie.
## 🌟 Główne funkcje
- **Inteligentne pogrubianie tekstu (Bionic Reading)**  
  Wtyczka automatycznie analizuje bloki tekstowe na stronach i pogrubia wybrane litery w wyrazach (np. pierwsze litery). Pomaga to "prowadzić" wzrok po tekście i pozwala mózgowi szybciej autouzupełniać słowa, zmniejszając zmęczenie. Masz możliwość wyboru w menu, czy pogrubiane mają być litery na początku, na końcu, czy po obu stronach słowa.
  
- **Filtrowanie szumu wizualnego**  
  Skrypt celowo ignoruje duże nagłówki (H1-H6), przyciski i linki nawigacyjne. Modyfikuje tylko ten tekst, który faktycznie wymaga skupienia (np. akapity i długie artykuły), zapobiegając w ten sposób "przebodźcowaniu".
- **Muzyka tła oparta na kontekście (Offline AI)**  
  Wtyczka posiada wbudowany analizator słów kluczowych. Błyskawicznie skanuje czytany tekst w poszukiwaniu kontekstu tematycznego i na jego podstawie dobiera odpowiedni, zapętlony dźwięk w tle (np. "klimat fantasy" dla tekstów fabularnych lub "szum fal/lo-fi" dla artykułów naukowych). Działa to całkowicie offline, bez wysyłania Twoich danych na zewnętrzne serwery.
## 🛠️ Instalacja dla testerów
Aby przetestować wtyczkę lokalnie w swojej przeglądarce:
1. Pobierz to repozytorium na swój komputer.
2. Otwórz przeglądarkę Google Chrome i przejdź pod adres: `chrome://extensions/`
3. W prawym górnym rogu włącz **Tryb programisty** (Developer mode).
4. W lewym górnym rogu kliknij **Załaduj rozpakowane** (Load unpacked).
5. Wybierz folder z wtyczką. Gotowe! (Zalecamy przypięcie ikony puzzla na pasku).
## ⚙️ Technologie
- Vanilla JavaScript, HTML5, CSS3
- Google Chrome Extension API (Manifest V3)
- Chrome Offscreen API (do odtwarzania ciągłego dźwięku w tle)
---
*Projekt stworzony z myślą o lepszym i spokojniejszym przyswajaniu wiedzy w cyfrowym świecie.*
---
